...
import { getHeroSectionData } from '../utils/localStorageManager';
...
const DownloaderTool = () => {
  ...
  // Hero section data from localStorage
  ...
  // Listen for hero section updates
  useEffect(() => {
    const updateHero = () => {
      const data = getHeroSectionData();
      if (data) {
    ...
    window.addEventListener('heroSectionUpdated', updateHero);
    return () => window.removeEventListener('heroSectionUpdated', updateHero);
  }, []);
